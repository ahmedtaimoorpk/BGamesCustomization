...
BlocklyDialogs.congratulationsPartsCollected = function () {
    ...
    if (BlocklyGames.workspace) {
        ...
        linesText.appendChild(document.createTextNode("Hurray, You have Collected all the missing Parts. Now Assemble the machine to fix it!"));
    }
...
/**
 * GASES GAME
 * after the game has been initialized show this dialog
 */
BlocklyDialogs.gasesGameStart = function () {
    var content = document.getElementById('dialogDone');
    var style = {
        width: '40%',
        left: '30%',
    ...
    content.getElementsByTagName("div")[0].innerHTML = "Hi There!";
    document.getElementById("doneOk").innerHTML = "PLAY";
    document.getElementById('doneCancel').remove();

    // Add the user's code.
    if (BlocklyGames.workspace) {
        var linesText = document.getElementById('dialogLinesText');
        linesText.textContent = '';
        linesText.appendChild(document.createTextNode("Wondering how this steam engine works? Click on the PLAY button to make it work."));
    }

    var cancel = document.getElementById('doneOk');
    cancel.addEventListener('click', BlocklyDialogs.gasesGameClose, true);
    cancel.addEventListener('touchend', BlocklyDialogs.hideDialog, true);
...
BlocklyDialogs.congratulations = function () {
    ...
    if (BlocklyGames.workspace) {
        var linesText = document.getElementById('dialogLinesText');
        linesText.textContent = '';
        linesText.appendChild(document.createTextNode("Hurray, You Solved this level."));
    }

    content.getElementsByTagName("div")[0].innerHTML = "Congratulations!";
    document.getElementById("doneOk").innerHTML = "NEXT";
